...
def save_descriptors_as_string(file_name, features):
	with open(file_name, 'w') as f:
	    #m = np.matrix(features)
	    savetxt(f, features, fmt='%s') #fmt='%s'
	print('Save Descriptors as Strings: Done')

def save_descriptors_as_matrix(file_name, features):
    with open(file_name, 'w') as f:
	    #m = np.matrix(features)
	    savetxt(f, features, fmt='%f') #fmt='%s'
    print(type(features))
    print('Save Descriptors as Floats: Done')
